...
#create the subroutine that does that actual reporting
 sub cb {
     ...
     if ( ${[%links]}[1] =~ m/\.aac/) { print "${[%links]}[1]\n"; }
     #print "${[%links]}[1]\n";
 }
 $p->parse_file("$ARGV[0]");
...
useage: B<aacgrep.pl> I<filename>

Scans a filename (or STDIN if piped) for html links to files ending in .aac

Written by john@kript.net.  Licensed under the GPL.

Check for updates at http://cgi.kript.net/blosxom.cgi/code/

Download the latest version; http://www.kript.net/perl/aacgrep.pl
