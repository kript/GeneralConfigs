...
output="/tmp/device_iostat_avgrq-sz_$1.csv"
...
for device in $dmlist
    do
        ...
        egrep "^$device\s" $1 | awk '{print $8}' >> /tmp/device_$device.txt
...
gnuplot << EOF
  ...
  set grid
  set output "/tmp/$1_$2_avgrq-sz.png"
  set title "iostat avgrq-sz"  
...
echo "created image at /tmp/$1_$2_avgrq-sz.png" 
